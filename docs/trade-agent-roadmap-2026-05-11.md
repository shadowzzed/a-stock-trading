...
| ~~P2~~ | ~~板块龙头加成提高（leader 3→4→5）~~ | ~~+1-2pp~~ | **实测 0pp ❌，选股排名稳定，加成不改变结果** | 1h | - |
| ~~P2~~ | ~~sealed_min_prev_board 收紧（1→2）~~ | ~~+1pp~~ | **实测 -3.87pp ❌，胜率升但笔数砍半（见 §3.1.0b）** | 1h | - |
| ~~P2~~ | ~~max_hold_days 缩短（5→3）~~ | ~~?~~ | **实测 -1.5pp ❌，胜率升但让赢家提前出场** | 1h | - |
...
## 5.4 实验总结（2026-05-11 实测）

5 个单点改动实验结果：

| 实验 | 预期 | 实测 | 结论 |
|------|------|------|------|
| 紧止损 -5/+10 | +3pp | **+9.68pp ✅** | 上线 |
| Layer1 deterministic 门控 | +2-4pp | **+5.94pp ✅** | 上线 |
| Trailing stop loss (+5%/3%) | +3pp | -1pp ❌ | 不上线 |
| sealed_min_prev_board=2 | +1pp | -3.87pp ❌ | 不上线 |
| BOARD_LEADER_BONUS 3→4→5 | +1-2pp | 0pp ❌ | 选股排名稳定，加成不变 |
| max_hold_days 5→3 | ? | -1.5pp ❌ | 让赢家提前出场 |

**核心洞察**：单点改动空间已被 v8_tight 占据。下一轮提升需要**系统性改动**：

1. **多策略组合**：v8_tight + v8_tight_fast 各 50% 仓位，分散风格
2. **加仓机制**：单标的 30%→35% 提升资金利用率
3. **样本扩展**：测 2025 年中长期窗口验证稳定性
4. **板块轮动**：Layer 1 输出 top_sectors，加权 Layer 2 选股
5. **分时段持仓**：高潮日加仓、退潮日减仓（动态 max_positions）

短期内 v8_tight 已经是局部最优，继续在熊+震荡混合市场维持 13% 月化。
