...
### 5.1 当前基准（2026-05-11 更新）

v8_tight（含 Layer1 deterministic）跨两个真实窗口实测：

| 窗口（21+18=39 个交易日） | 收益 | 笔数 | 胜率 |
|---------|------|------|------|
| 熊市 3-12 ~ 4-10 | -1.49% | 12 | 33.3% |
| 震荡 4-13 ~ 5-08 | +18.26% | 24 | 50.0% |
| **合计（约 1.85 个月）** | **+16.5%** | **36** | **44%** |

月化估值 ~8.9%（37 个交易日 +16.5%），低于 20% 目标但已经覆盖了熊+震荡两种行情。

| 情景 | 假设 | 月化预期 |
|------|------|---------|
| 乐观 | 升温/高潮市 + 趋势股放量 | **+25-30%** |
| 中性 | 震荡市占比 60%，熊市 40% | **+5-10%** |
| 悲观 | 极端熊市连续 1 个月 | **-3 ~ +2%**（Layer1 保护下） |

**关键差异**：v8_base（旧生产）熊市会亏 -15%+，v8_tight + Layer1 熊市只亏 -1.49%。**回撤改善 10pp+**，但单月 20% 目标在中性市场难达成，需要叠加 P1 因子改进（试错止损、龙头加成）。
