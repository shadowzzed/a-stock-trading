...
shadow_runner 18 个交易日（2026-04-13 ~ 2026-05-08）8 策略对比：

| 排名 | 策略 | 收益 | 笔数 | 胜率 | 平均/笔 | 回撤 | 状态 |
|------|------|------|------|------|--------|------|------|
| 🥇 | **v8_tight** | **+15.32%** | 21 | 52.4% | 2.13% | 10.1% | ⚠️ warning |
...
| 优先级 | 改动 | 预期月化提升 | 工作量 | 风险 |
|--------|------|------------|------|------|
| P0 | 移动止盈（trailing_activate=5/drawdown=3） | +3pp | 2h | 低，已有逻辑 |
| P1 | 入场后第 1 分钟试错止损（-3% 立即出） | +2pp | 4h | 中，可能误杀真信号 |
| P1 | Layer 1 GLM 门控（layer1_gate=True） | +2-4pp | 0h | 已有，仅需切配置 |
| P2 | 板块龙头加成提高（leader 2→3） | +1-2pp | 1h | 低 |
| P2 | sealed_min_prev_board 收紧（1→2） | +1pp | 1h | 减少噪音封板 |
| P3 | 多空过滤（连续 3 天下跌则空仓） | +2pp | 6h | 高，需新数据流 |
