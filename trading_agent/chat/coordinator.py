...
from .agents.auction import AuctionAgent
...
- bullbear: 交易员（主线判断、板块轮动、资金流向、策略）
- trend: 趋势分析师（均线系统、量价背离、支撑压力位、技术面、趋势股扫描）
- auction: 竞价辨识度分析师（竞价筛选、高开过顶、断板反包、超预期首板、竞价三信号）
...
- 竞价/开盘辨识度/竞价筛选/高开过顶/断板反包/竞价信号 → ["auction"]
- 今日竞价怎么看/竞价分析 → ["auction", "sentiment", "bullbear"]
...
class CoordinatorAgent(BaseAgent):
    ...
    def __init__(self, data_dir: str, memory_dir: str,
        ...
        self.trend = TrendAgent(data_dir, memory_dir, cache=cache,
                                backtest_max_date=backtest_max_date)
        self.auction = AuctionAgent(data_dir, memory_dir, cache=cache,
                                    backtest_max_date=backtest_max_date)
